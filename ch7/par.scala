...
object Par {
  ...
  def sequence_simple[A](as: List[Par[A]]): Par[List[A]] = {
    as.foldRight[Par[List[A]]](unit(List[A]()))((a, acc) => map2(a, acc)(_ :: _))
  }
  ...
  def parFilter[A](as: List[A])(f: A => Boolean): Par[List[A]] = {
    as.foldRight[Par[List[A]]](unit(List[A]()))((a, acc) => if(f(a)) map2(unit(a), acc)(_ :: _) else acc)
  }
